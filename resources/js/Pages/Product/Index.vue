        ...
        <div class="px-10 py-7">
            ...
            <div class="flex justify-between items-center mx-3">
                ...
                <div class="flex items-center space-x-3">
                    ...
                    <PrimaryButton @click="$inertia.get(route('products.create'))" class="!rounded-full">Nuevo producto</PrimaryButton>
                </div>
            </div>

            <div class="lg:w-1/4 relative">
                <input v-model="searchQuery" @keydown.enter="searchProducts" class="input w-full pl-9" placeholder="Buscar producto" type="text">
                <i class="fa-solid fa-magnifying-glass text-xs text-gray99 absolute top-[10px] left-4"></i>
            </div>
            
            <div class="mt-8 w-11/12">
                <Pagination :pagination="products" />
                <ProductTable :products="productsFound" />
            </div>
        </div>
        ...
        <h1 class="font-bold my-4">Ingresar producto a almacén</h1>
          <section class="text-center mt-5 mb-2 mx-5">
            <div class="mt-3 col-span-2">
                    ...
                    <el-input v-model="form.code" @keydown.enter="getProduct" ref="codeInput" placeholder="Escanea el código del producto" :maxlength="100" clearable>
                        <template #prefix>
                ...
                <div v-if="productEntryFound?.length > 0" class="mt-3">
                    ...
                    <el-input v-model="form.quantity" ref="quantityInput" autofocus @keydown.enter="entryProduct" placeholder="Catidad que entra a almacén"
                ...
                <div v-if="productEntryFound?.length > 0" class="mt-5 grid grid-cols-3">
                    <figure class="w-32 ml-16">
                        <img class="w-32 object-contain" :src="productEntryFound[0]?.imageCover[0]?.original_url" alt="">
                    </figure>
                ...
                <p v-else-if="!loading && form.code" class="mt-5 text-gray-500 text-center text-sm">No se encontró ningun producto</p>
...
data() {
    ...
    return {
       ...
       productsFound: this.products.data
    };
},
components:{
AppLayout,
    ...
    async getProduct() {
     ...
     entryProduct() {
       this.form.put(route('products.entry', this.productEntryFound[0]?.id), {
        onSuccess: () => {
            const IndexProductEntry = this.productsFound.findIndex(item => item.code === this.form.code);
            this.productsFound[IndexProductEntry].current_stock += parseInt(this.form.quantity); 
            this.form.reset();
