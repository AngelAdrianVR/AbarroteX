<template>
        <section v-if="Object.keys(items)?.length" class="mt-4 text-sm w-full overflow-auto">
            <table class="w-full">
                ...
                <tr @click="$inertia.visit(route('cash-cuts.show', index))" class="*:py-2 *:px-4 text-xs cursor-pointer hover:bg-primarylight" v-for="(item, index) in items" :key="item">
                    ...
                    <td :class="differenceStyles(item.total_difference)">${{ item.total_difference?.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}</td>
