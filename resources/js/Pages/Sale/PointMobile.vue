    ...
    <main class="pt-1" :class="getHeightClass">
      ...
      <section class="overflow-auto px-2 lg:px-6" :class="showNoCodeProducts ? 'h-[65%]' : 'h-[94%]'">
          ...
          <section class="flex items-center justify-between">
            ...
            <article class="flex items-center space-x-1">
              <button type="button" @click="showSearchModal = true"
                class="px-4 py-1 bg-grayF2 rounded-full text-gray37 flex items-center space-x-1 text-xs">
              ...
              <div v-if="$page.props.auth.user.permissions.includes('Registrar movimientos de caja')">
                <el-col :span="3">
                  <el-dropdown trigger="click">
                    <button class="rounded-full size-6 flex items-center justify-center bg-[#EDEDED] text-gray37">
        ...
        <div class="my-2">
          <div>
            <!-- scaner de código  -->
            <div v-if="isScanOn" class="relative w-4/5 mx-auto">
              <input v-model="scannerQuery" :disabled="scanning || syncingIDB" ref="scanInput"
                class="input !h-7 w-full pl-9" placeholder="Escanea o teclea el código del producto" type="text">
              <i class="fa-solid fa-barcode text-xs text-gray99 absolute top-[7px] left-4"></i>
            </div>
            <!-- Pestañas -->
            <div class="mt-3">
              <el-tabs v-model="editableTabsValue" type="card" class="demo-tabs">
                <div v-if="$page.props.auth.user.store.activated_modules.includes('Clientes')"
                  ...
                  <div class="flex items-center space-x-3 w-3/4 md:w-1/2 text-sm">
                    <p class="flex items-center space-x-1">
                      <svg width="11" height="10" viewBox="0 0 11 10" fill="none" xmlns="http://www.w3.org/2000/svg"
                        class="size-4">
                    ...
                    <el-select v-model="editableTabs[editableTabsValue - 1].client_id" clearable filterable
                      ...
                      no-match-text="No se encontraron coincidencias" size="small">
      ...
      <section v-if="isQuickNoCodeSelectionOn && noCodeProducts.length"
        ...
        <div v-if="showNoCodeProducts" class="mt-2 px-3 py-1 overflow-auto h-[82%]">
          ...
          <div v-show="showNoCodeProducts" class="grid grid-cols-2 md:grid-cols-4 gap-2">
            <button @click="handleFastProductSelection(item)" type="button"
              ...
              <figure class="flex items-center justify-center h-10">
                <img v-if="item.image_url" :src="item.image_url" :alt="item.name" class="object-contain h-10 mx-auto">
      ...
      <section v-if="editableTabs[editableTabsValue - 1]?.saleProducts?.length"
        ...
        <div v-if="editableTabs[editableTabsValue - 1]?.credit">
          <div class="flex items-center justify-between">
            ...
            <p class="text-gray-99 text-base">$ <strong class="ml-3">{{
              calculateTotal().toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g,
                ",") }}</strong></p>
          </div>
          <h3 class="text-sm font-bold">Registrar abono de la venta</h3>
          ...
          <div class="w-2/3 pr-5">
            <InputLabel value="Fecha de vencimiento (opcional)" class="!text-sm !text-gray-400 ml-2" />
    ...
    <Modal :show="cashCutModal" @close="cashCutModal = false; form.reset">
      <div class="p-4 relative">
        ...
        <form class="mt-5 mb-2" @submit.prevent="storeCashCut">
          ...
          <p class="col-span-full">Por favor, cuenta el efectivo en caja y verifica el monto total en tu tarjeta. Ingresa ambos valores para realizar el corte.</p>
          ...
          <section class="w-full flex justify-end space-x-3 text-sm border-b border-gray-200 pb-3">
            ...
            <div class="w-44 space-y-2">
              ...
              <div class="space-y-[8px]" v-else>
                <div class="flex justify-between">
                  ...
                  <p>{{ (asignedCashRegister.started_cash 
    ...
    <DialogModal :show="showSearchModal" @close="showSearchModal = false" max-width="xl">
      ...
      <template #content>
        <el-autocomplete v-model="productFoundSelectedName" clearable :fetch-suggestions="searchProducts"
          class="!w-full" placeholder="Escribe el nombre del producto" @select="handleSelectFoundProduct()"
          :value-key="'name'" :loading="loading" :disabled="syncingIDB || scanning" ref="searchInput">
          <template #prefix>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
              stroke="currentColor" class="size-4 text-gray99">
              <path stroke-linecap="round" stroke-linejoin="round"
                d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" />
            </svg>
          </template>
          ...
          <template #loading>
            <SmallLoading class="mx-auto my-4" />
          </template>
        </el-autocomplete>
        <!-- Detalle de producto encontrado -->
        <section class="border border-grayD9 rounded-[10px] p-1 mt-2 text-xs">
          <div v-if="productFoundSelected" class="grid grid-cols-3 gap-x-4 gap-y-1 items-start">
            <figure class="rounded-[10px] h-28 flex items-center justify-center self-start">
              <img v-if="productFoundSelected.imageUrl" :draggable="false"
                class="mx-auto h-full object-contain select-none rounded-[10px]" :src="productFoundSelected.imageUrl"
                :alt="productFoundSelected.name">
            ...
            <div class="col-span-2 flex flex-col space-y-1 justify-center py-2">
              <h2 class="font-bold text-sm text-gray37">
                <span>{{ productFoundSelected.name }}</span>
              ...
              <p :class="productFoundSelected.bulk_product ? 'text-gray-900' : 'text-[#5FCB1F]'">
                ...
                <span v-if="productFoundSelected.bulk_product" class="text-gray-800">
                  /{{ productFoundSelected.measure_unit === 'Kilogramo' ? 'Kg' : 'L' }}
                </span>
              </p>
            ...
            <div class="col-span-full">
              <p v-if="productFoundSelected.current_stock == 0 && isInventoryOn"
                class="text-sm text-gray99 text-center">
...
export default {
  data() {
    ...
    return {
      ...
      // modales
      showSearchModal: false, // muestra modal para buscar producto por nombre
  ...
  methods: {
    ...
    differenceCash() {
      ...
      this.cutForm.difference_cash = (
          ...
          this.asignedCashRegister.started_cash) - 
