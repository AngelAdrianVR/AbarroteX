    ...
    <main class="w-full mx-auto h-screen flex flex-col justify-between overflow-hidden">
        <div>
            ...
            <section class="flex justify-between items-center">
                <div class="relative">
                    <svg width="500" height="109" class="text-white" viewBox="0 0 577 125" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <path d="M392.167 125L577 0L0 0V125L392.167 125Z" fill="#373B46" />
                    </svg>
                    <p class="text-white font-bold text-3xl absolute top-9 left-14 font-sans">{{
                        $page.props.auth.user.store.name }}</p>
                </div>
            ...
            <body class="my-4 md:w-[95%] lg:w-[80%] mx-auto">
                ...
                <!-- cliente -->
                <p class="font-bold" v-if="quote.client?.name">Cliente: <span class="font-thin">{{ quote.client?.name
                        }}</span></p>
                <p class="font-bold">Contacto: <span class="font-thin">{{ quote.contact_name }}</span></p>
                <!-- dirección -->
                <p class="font-bold" v-if="quote.address">Dirección: <span class="font-thin">{{ quote.address }}</span>
                </p>
                ...
                <table class="w-full my-5">
                    ...
                    <tbody>
                        ...
                        <tr v-for="(product, index) in quote.products" :key="index"
                            ...
                            <td>{{ product.name }}</td>
                            <td>-</td>
                            <td>${{ (product.price / 1.16)?.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") ?? '-' }}
                            </td>
                            <td>{{ product.quantity?.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") ?? '-' }}</td>
                            <td>${{ ((product.price / 1.16) *
                                product.quantity).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") ?? '-' }} + IVA</td>
                        </tr>
                        <!-- servicios -->
            ...
            <section class="flex flex-col space-y-1 text-base -mb-10">
                ...
                <div class="flex items-center space-x-2">
                    <div class="border border-[#F68C0F] rounded-full">
                        <svg class="m-1" width="22" height="22" viewBox="0 0 12 12" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <mask id="mask0_11240_150" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="0" y="0"
                                width="12" height="12">
                ...
                <div class="flex items-center space-x-2">
                    <div class="border border-[#F68C0F] rounded-full">
                        <svg class="m-1" width="22" height="22" viewBox="0 0 12 12" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <mask id="mask0_11240_170" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="0" y="0"
                                width="12" height="12">
                ...
                <div class="flex items-center space-x-2">
                    <div class="border border-[#F68C0F] rounded-full">
                        <svg class="m-1" width="22" height="22" viewBox="0 0 12 12" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <mask id="mask0_11240_159" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="0" y="0"
                                width="12" height="12">
                                <rect width="12" height="12" fill="#D9D9D9" />
                            </mask>
