<template>
    <AppLayout :title="'Orden-' + String(report.folio).padStart(3, '0')">
        <main class="py-4 px-3 lg:px-8">
            <Back />
            <h1 class="text-2xl font-bold">Orden de servicio No. {{ String(report.folio).padStart(3, '0') }}</h1>
            <div class="flex justify-between mt-3 mb-5">
                <p class="text-[#999999]">Fecha de recepción: <span class="text-black">{{ formatDate(report.created_at) }}</span></p>
                <el-dropdown split-button trigger="click" type="primary" @click="$inertia.get(route('service-reports.edit', encodeId(report.id)))">
                    Editar
                    <template #dropdown>
                    <el-dropdown-menu>
                        <el-dropdown-item>
            ...
            <body class="grid grid-cols-4 gap-4">
                <section class="col-span-3">
                    <StatusStep :activeStep="statuses.findIndex(status => status === report.status) + 1" :steps="statuses">
                        <template #icon-0>
                    ...
                    <div class="flex justify-between my-2 ml-4">
                        <button class="flex items-center text-[#373737] hover:underline">
                            <svg class="size-[14px] mr-2" width="13" height="13" viewBox="0 0 13 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <PrimaryButton @click="changeStatus" v-if="report.status !== 'Entregado/pagado'" 
                            class="!bg-gradient-to-r from-[#751F8B] via-[#754681] to-[#1F0825]"
                    ...
                    <div class="mt-5 ml-4 text-[15px]">
                        ...
                        <div class="flex space-x-4 py-2 px-1">
                            ...
                            <p class="lg:w-1/2">{{ report.comision_percentage ?? '-' }}%</p>
                        </div>
                ...
                <!-- Lado derecho -->
                <section>
                    <div class="border border-[#D9D9D9] rounded-lg p-4 space-y-2 h-[600px]">
                        ...
                        <div class="flex justify-between">
                            ...
                            <button class="text-primary bg-primarylight rounded-full px-3 text-sm">Enviar mensaje</button>
                        </div>
...
methods:{
    ...
    async changeStatus() {
        ...
        try {
            const response = await axios.post(route('service-reports.change-status', this.report.id), { status: newStatus });
