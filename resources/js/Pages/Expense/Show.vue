<template>
    <AppLayout :title="'Gastos del día'">
        ...
        <main class="mx-2 lg:mx-32 text-sm mt-4">
...
<div class="mt-7 lg:mx-16">
    <p class="font-bold px-2">Fecha: <span class="font-thin ml-2 text-gray-600">{{ formatDate(expenses[0]?.created_at)
            }}</span></p>
...
<div class="grid grid-cols-5 lg:ml-16 mr-3 self-start mt-9">
    ...
    <div class="mt-2">
        <p class="text-sm" v-for="expense in expenses" :key="expense">${{
            expense.current_price?.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}</p>
    </div>
    <div class="mt-2 ml-8">
        <p class="text-sm" v-for="expense in expenses" :key="expense">${{ (expense.quantity *
            expense.current_price).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}</p>
    </div>
    <div class="border-b border-primary w-28 col-start-5 my-3"></div>
    <p class="col-start-5 text-sm font-bold">Total: <span class="ml-2">${{
            totalExpenses().toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}</span></p>
</div>
</div> -->
    </AppLayout>
