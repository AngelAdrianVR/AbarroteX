    ...
    <div class="overflow-auto">
        <div class="lg:flex items-center lg:space-x-2">
            ...
            <p class="text-xs text-gray37 mt-1 lg:mt-0">
                {{ clients.length }} {{ clients.length == 1 ? 'elemento' : 'elementos' }} en la tabla
            </p>
        </div>
        <el-table ref="tableRef" :data="clients" @row-click="handleRowClick" max-height="500"
            :row-class-name="tableRowClassName" :default-sort="{ prop: 'debt', order: 'descending' }"
...
export default {
    ...
    methods: {
        ...
        handleRowClick(row) {
            this.$inertia.get(route('clients.show', this.encodeId(row.id)));
        },
