      ...
      <div class="mt-1">
        ...
        <article v-for="item in modules" :key="item" class="flex items-center space-x-1 border border-grayD9 text-white rounded-md py-3 px-4 mb-2">
            <div class="w-3/4">
                ...
                <p class="text-gray99 mt-2">{{ item.description }}</p>
                <p class="my-3">${{ item.cost }}.00/mes</p>
            </div>
    ...
    <section class="lg:w-1/2 lg:pl-12 mt-9 lg:mt-0">
      ...
      <article class="rounded-xl border border-grayD9 p-4 mt-2">
        ...
        <div class="flex items-center justify-between space-x-4">
            <!-- boton de mensual -->
            <div @click="period = 'Mensual'" :class="period === 'Mensual' ? 'bg-primarylight text-primary border-primary' : 'bg-transparent text-white border-grayD9'" 
            class="cursor-pointer rounded-lg border-2 p-3 font-bold w-full">
                <p>Mensual</p>
            </div>
            <!-- boton de Anual -->
            <div @click="period = 'Anual'" :class="period === 'Anual' ? 'bg-primarylight text-primary border-primary' : 'bg-transparent text-white border-grayD9'" 
            class="cursor-pointer rounded-lg border-2 p-3 font-bold w-full relative">
                <!-- popular -->
                <div class="absolute -top-3 right-2 rounded-full bg-[#006847] text-white flex items-center space-x-2 text-[10px] px-2 py-[2px]">
                    <i class="fa-solid fa-star"></i>
                    <p>Popular</p>
                </div>
                <p>Anual <span :class="period === 'Anual' ? 'text-[#494949]' : 'text-white'" class="text-xs ml-2 font-thin">2 meses de regalo</span></p>
            </div>
        </div>
        ...
        <div class="mt-7 text-grayD9 px-3">
            <div class="flex">
                ...
                <p class="w-1/4 text-right"><span class="mr-1">$</span><span class="w-20 inline-block">{{ period === 'Mensual' ? '199.00' : '1,990.00' }}</span></p>
            </div>
            ...
            <div v-for="item in modules.filter(item => item.activated === true)" :key="item" class="flex">
                ...
                <p class="w-1/4 text-right"><span class="mr-1">$</span><span class="w-20 inline-block">{{ period === 'Mensual' ? item.cost.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") : (item.cost * 10).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}</span></p>
            </div>

            <!-- Total -->
            <div class="flex text-white mt-7">
                <p class="w-3/4">Total</p>
                <p class="w-1/4 text-right"><span class="mr-1">$</span><span class="w-20 inline-block">{{ period === 'Mensual' ? calculateTotal.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") : (calculateTotal * 10).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",")  }}</span></p>
            </div>
