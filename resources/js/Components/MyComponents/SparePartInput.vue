<template>
  <main class="py-3 pl-3 border rounded-md">
    <section class="max-h-60 overflow-auto">
      <div v-if="parts.length" class="grid grid-cols-9 gap-4 items-center mb-4 relative overflow-hidden"
        v-for="(part, index) in parts" :key="index">
        <div class="col-span-3">
          <div class="flex items-center justify-between">
            <InputLabel value="Nombre de la refacción" />
            <p v-if="index === (parts.length - 1)" @click="changeTypeOfSelection"
              class="cursor-pointer text-primary underline text-sm">Cambiar a {{ typeOfSelection === 'texto' ? 'lista' :
                'texto' }}</p>
          </div>
          ...
          <div v-else>
            <template v-if="index === parts.length - 1">
              ...
              <el-input v-if="typeOfSelection === 'texto'" v-model="part.name" placeholder="Nombre de la refacción" />
              <el-select v-else v-model="part.name" @change="handleSparePartSelected(index, $event)" clearable
                placeholder="Selecciona la refacción" no-data-text="No hay opciones registradas"
                no-match-text="No se encontraron coincidencias">
                <el-option v-for="option in spareParts" :key="option.id" :label="option.name" :value="option.name" />
              </el-select>
              <p v-if="index === parts.length - 1 && nameError" class="text-xs text-red-500">
        ...
        <div class="col-span-2">
          ...
          <el-input v-model.number="part.unitPrice" @input="syncItems()">
        ...
        <div class="col-span-2">
          ...
          <div class="flex items-center gap-1">
            <el-input-number v-model="part.quantity" @change="syncItems()" :min="1" />
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    addPart() {
      ...
      // Si no hay partes, simplemente agrega una nueva y termina.
      if (!lastPart) {
        this.parts.push({ name: '', unitPrice: 0, quantity: 1 });
        return;
      }
      ...
      // Valida el nombre de la última parte
      if (lastPart.name.trim() === '') {
        this.nameError = true;
        return; // Detiene la ejecución si hay un error
      }

      // Valida el precio de la última parte
      if (!lastPart.unitPrice) {
        this.priceError = true;
        return; // Detiene la ejecución si hay un error
      }

      // Si todas las validaciones pasan, agrega la nueva parte
      this.parts.push({
        name: '',
        unitPrice: 0,
        quantity: 1,
