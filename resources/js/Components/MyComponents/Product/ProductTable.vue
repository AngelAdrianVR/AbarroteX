<template>
    <div>
        ...
        <ConfirmationModal :show="showDeleteConfirm" @close="showDeleteConfirm = false">
            ...
            <template #content>
                <p>
                    Se eliminará el producto seleccionado, esto es un proceso irreversible. ¿Continuar
                    de todas formas?
                </p>
            </template>
            <template #footer>
    ...
    <!-- <div v-if="Object.keys(products)?.length" class="w-full mx-auto text-[11px] md:text-sm overflow-auto">
        ...
        <div>
            <div v-for="product in products" :key="product.id"
                class="*:px-2 *:py-1 cursor-pointer flex items-center hover:border-primary space-x-4 border rounded-full mb-2"
                ...
                <div class="w-[18%] md:w-[13%]">{{ product.global_product_id ? product.global_product?.code :
                    product.code }}</div>
                ...
                <div class="rounded-r-full w-[17%] text-right">
                    <i @click.stop="handleEdit(product)"
                    ...
                    <el-popconfirm confirm-button-text="Si" cancel-button-text="No" icon-color="#C30303"
                        title="¿Continuar?" @confirm="deleteItem(product)">
                        <template #reference>
                            <i @click.stop
