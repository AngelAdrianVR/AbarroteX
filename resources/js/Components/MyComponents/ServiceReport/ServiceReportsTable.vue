<template>
    <div class="overflow-auto">
        ...
        <table v-if="reports?.length" class="w-full table-fixed">
            ...
            <tbody>
                <tr @click="openTemplate(report.id)" v-for="(report, index) in internalReports" :key="index"
                    class="*:text-xs *:py-2 *:px-4 hover:bg-primarylight cursor-pointer">
                    <td class="rounded-s-full" @click.stop>
                        <el-checkbox :model-value="selectedReports.includes(report.id)"
                                    ...
                                    <div class="text-sm">
                                        ...
                                        <div v-if="report.status === 'Cancelada'">
                                            <p class="text-blue-300">
                                                Razón: <span class="text-white">{{ report.cancellation_reason ?? '-'
                                                    }}</span>
                                            </p>
                                            <p class="text-blue-300">
                                                Monto de revisión: <span class="text-white">${{
                                                    report.aditionals?.review_amount ?? '0.00' }}</span>
                                            </p>
                                            <p class="text-blue-300">
                                                Adelanto regresado: <span class="text-white">${{
                                                    report.aditionals?.advance_amount ?? '0.00' }}</span>
                                            </p>
                                        </div>
                                        <div v-if="report.status === 'Entregado/Pagado'">
                                            <p class="text-green-300">
                                                Pagado el: <span class="text-white">{{ formatDate(report.paid_at) ?? '-'
                                                    }}</span>
                                            </p>
                                        </div>
                                    </div>
                        ...
                        <el-dropdown trigger="click" @command="handleCommand">
                            ...
                            <template #dropdown>
                                <el-dropdown-menu>
                                    ...
                                    <el-dropdown-item
                                        v-if="canEdit && report.status !== 'Cancelada' && report.status !== 'Entregado/Pagado'"
                                        :command="'edit|' + encodeId(report.id)">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            stroke-width="1.5" stroke="currentColor" class="size-[14px] mr-2">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                    ...
                                    <div v-if="canEdit">
                                        <p v-if="report.status !== 'Cancelada' && report.status !== 'Entregado/Pagado'"
                                            class="text-gray-400 text-xs text-center px-3 my-1">Cambiar estatus</p>
                                        <el-dropdown-item
                                            v-if="report.status !== 'Cancelada' && report.status !== 'Entregado/Pagado'"
                                            :command="'changeStatus|' + report.id + '|Recibida'">
                                        ...
                                        <el-dropdown-item
                                            v-if="report.status !== 'Cancelada' && report.status !== 'Entregado/Pagado'"
                                            :command="'changeStatus|' + report.id + '|En proceso'">
                                        ...
                                        <el-dropdown-item
                                            v-if="report.status !== 'Cancelada' && report.status !== 'Entregado/Pagado'"
                                            :command="'changeStatus|' + report.id + '|Listo para entregar'">
                                        ...
                                        <el-dropdown-item
                                            v-if="report.status !== 'Cancelada' && report.status !== 'Entregado/Pagado'"
                                            :command="'changeStatus|' + report.id + '|Entregado/Pagado'">
