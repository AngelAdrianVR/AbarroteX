        ...
        <table v-if="reports?.length" class="w-full table-fixed">
            ...
            <tbody>
                <tr @click="openTemplate(report.id)"
                    v-for="(report, index) in internalReports" :key="index"
                    class="*:text-xs *:py-2 *:px-4 hover:bg-primarylight cursor-pointer">
                    <td class="rounded-s-full" @click.stop>
                        <el-checkbox
                            :model-value="selectedReports.includes(report.id)"
                            @change="val => handleCheckboxChange(val, report.id)"
                        />
                    </td>
                    <td class="mt-[5px]">
                                    ...
                                    <div class="text-sm">
                                        ...
                                        <div v-if="report.status === 'Cancelada'">
                                            <p class="text-blue-300">
                                                Razón: <span class="text-white">{{ report.cancellation_reason ?? '-' }}</span>
                                            </p>
                                            <p class="text-blue-300">
                                                Monto de revisión: <span class="text-white">${{ report.aditionals?.review_amount ?? '0.00' }}</span>
                                            </p>
                                            <p class="text-blue-300">
                        ...
                        <el-dropdown trigger="click" @command="handleCommand">
                            ...
                            <template #dropdown>
                                <el-dropdown-menu>
                                    ...
                                    <el-dropdown-item v-if="canEdit && report.status !== 'Cancelada' && report.status !== 'Entregado/Pagado'" :command="'edit|' + encodeId(report.id)">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            stroke-width="1.5" stroke="currentColor" class="size-[14px] mr-2">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                    ...
                                    <p v-if="report.status !== 'Cancelada' && report.status !== 'Entregado/Pagado'" class="text-gray-400 text-xs text-center px-3 my-1">Cambiar estatus</p>
                                    <el-dropdown-item v-if="report.status !== 'Cancelada' && report.status !== 'Entregado/Pagado'" :command="'changeStatus|' + report.id + '|Recibida'">
                                        <svg class="size-[14px] mr-2" width="17" height="13" viewBox="0 0 17 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <el-dropdown-item v-if="report.status !== 'Cancelada' && report.status !== 'Entregado/Pagado'" :command="'changeStatus|' + report.id + '|En proceso'">
                                        <svg class="size-[14px] mr-2" width="13" height="16" viewBox="0 0 13 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <el-dropdown-item v-if="report.status !== 'Cancelada' && report.status !== 'Entregado/Pagado'" :command="'changeStatus|' + report.id + '|Listo para entregar'">
                                    ...
                                    <el-dropdown-item v-if="report.status !== 'Cancelada' && report.status !== 'Entregado/Pagado'" :command="'changeStatus|' + report.id + '|Entregado/Pagado'">
            ...
            <div v-if="paymentModalStep === 3" class="py-4 px-7 relative">
                ...
                <section class="mx-auto mt-2 md:w-2/3">
                    <div v-if="!paymentConfirmed">
                        ...
                        <div class="flex justify-center mt-7">
                            <PrimaryButton @click="changeStatus('Entregado/Pagado', reportSelected.id)" :disabled="updatingStatus"
                                class="!px-20">
                                <i v-if="updatingStatus"
                                    class="fa-sharp fa-solid fa-circle-notch fa-spin mr-2 text-white"></i>
                                Confirmar pago
