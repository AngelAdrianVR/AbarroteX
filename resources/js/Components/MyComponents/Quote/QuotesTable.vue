    ...
    <div class="overflow-auto">
        <div class="lg:flex items-center lg:space-x-2">
            ...
            <p class="text-xs text-gray37 mt-1 lg:mt-0">
                {{ quotes.length }} {{ quotes.length == 1 ? 'elemento' : 'elementos' }} en la tabla
            </p>
        </div>
        <el-table ref="tableRef" :data="quotes" @row-click="handleRowClick" max-height="500"
            :row-class-name="tableRowClassName" :default-sort="{ prop: 'folio', order: 'descending' }"
            class="!w-full mx-auto">
            <el-table-column fixed sortable prop="folio" label="Folio" width="180" :filters="[
                { text: 'Sin enviar a cliente', value: 'Sin enviar a cliente' },
                { text: 'Esperando respuesta', value: 'Esperando respuesta' },
                { text: 'Autorizada', value: 'Autorizada' },
                { text: 'Rechazada', value: 'Rechazada' },
                { text: 'Pago parcial', value: 'Pago parcial' },
                { text: 'Pagado', value: 'Pagado' },
            ]" :filter-method="filterStatus">
                <template #default="scope">
                    <div class="flex items-center space-x-1">
                        <el-tooltip :content="scope.row.status" placement="right">
                            <span v-html="getStatusIcont(scope.row.status)"></span>
                        </el-tooltip>
                        <span>{{ 'C-' + String(scope.row.folio).padStart(4, '0') }}</span>
                    </div>
                </template>
            </el-table-column>
            <el-table-column prop="created_at" sortable label="Fecha de creación" width="200">
                <template #default="scope">
                    <p>{{ formatDate(scope.row.created_at) }}</p>
                </template>
            </el-table-column>
            <el-table-column prop="company" label="Cliente" width="200">
                <template #default="scope">
                    <p>
            ...
            <el-table-column label="Total" width="200">
                <template #default="scope">
                    <p>$ {{ grandTotal(scope.row).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") ?? '-' }}</p>
                </template>
            </el-table-column>
            <el-table-column prop="remaining" label="Saldo pendiente" sortable>
                <template #default="scope">
                    <p>
                        {{ scope.row.remaining
                            ? '$ ' + scope.row.remaining.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",")
                            : '-' }}
                    </p>
                </template>
            </el-table-column>
            <el-table-column align="right">
                <template #default="scope">
                    <el-dropdown trigger="click" @command="handleCommand">
                        ...
                        <template #dropdown>
                            <el-dropdown-menu
                                :class="canChangeStatus && scope.row.status != 'Pagado' ? '!w-[210px]' : null">
                                ...
                                <el-dropdown-item v-if="canEdit" :command="'edit|' + encodeId(scope.row.id)">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" class="size-[14px] mr-2">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                <el-dropdown-item
                                    v-if="canDelete && !['Pagado', 'Pago parcial'].includes(scope.row.status)"
                                ...
                                <div v-if="canChangeStatus && !['Pagado', 'Pago parcial'].includes(scope.row.status)">
                                    <p class="my-1 text-center text-xs text-gray99">Cambiar estado de cotización</p>
                                    <el-dropdown-item v-if="scope.row.status != 'Pago parcial'"
                                        :command="'status|' + scope.row.id + '|Sin enviar a cliente'">
                                    ...
                                    <el-dropdown-item v-if="scope.row.status != 'Pago parcial'"
                                        :command="'status|' + scope.row.id + '|Esperando respuesta'">
                                    ...
                                    <el-dropdown-item v-if="scope.row.status != 'Pago parcial'"
                                        :command="'status|' + scope.row.id + '|Autorizada'">
                                    ...
                                    <el-dropdown-item :command="'status|' + scope.row.id + '|Pago parcial'">
                                    ...
                                    <el-dropdown-item v-if="scope.row.status != 'Pago parcial'"
                                        :command="'status|' + scope.row.id + '|Pagado'">
                                    ...
                                    <el-dropdown-item :command="'status|' + scope.row.id + '|Rechazada'">
                                        <svg width="15" height="15" viewBox="0 0 13 13" fill="none" class="mr-2"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                                ...
                                <div v-else-if="scope.row.status == 'Pago parcial'">
                                    <el-dropdown-item :command="'status|' + scope.row.id + '|Pago parcial'">
                                        <svg width="13" height="13" viewBox="0 0 13 13" fill="none" class="mr-2"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path fill-rule="evenodd" clip-rule="evenodd"
